...
-- Seed inicial — modelo de 4 tiers (sem FREE), alinhado às migrations 31/32/43.
-- NOTA: este bootstrap reflete slug/level/nome/flags corretos para os gates
-- funcionarem num ambiente novo. Os PREÇOS/CICLOS finais (semestral/anual) são
-- definidos pela migration 43_plans_cycles (colunas extras). O ideal é gerar
-- este schema.sql via `pg_dump --schema-only` do banco pós-migrations; até lá,
-- após rodar este bootstrap, aplique as migrations 31→47 para preços/ciclos.
insert into public.plans (slug, name, level, price_cents, asaas_value, asaas_description, cashback_pct, store_discount_pct, estetica_discount_pct, features, sort_order) values
  ('start',          'Treino',         1,   3990,  39.90,
   'Kath Treinos — Treino: Biblioteca completa de treinos', 3,  8, 0,
   '{"workouts":true}'::jsonb, 1),
  ('evolucao',       'Performance',    2,   7490,  74.90,
   'Kath Treinos — Performance: Treinos + Plano alimentar personalizado', 5, 12, 0,
   '{"workouts":true,"diet":true,"evolution_tracking":true}'::jsonb, 2),
  ('saude_completa', 'Saúde Completa', 3,   9990,  99.90,
   'Kath Treinos — Saúde Completa: Treinador + Anamnese + Suplementação', 7, 18, 0,
   '{"workouts":true,"diet":true,"supplements":true,"anamnese_based_diet":true,"chat_sla_h":48,"chat_priority":true,"reavaliation":"monthly","hydration_alerts":true,"daily_motivation":true,"vip_group":true,"premium_content":true}'::jsonb, 3),
  ('atleta',         'Atleta',         4,  30990, 309.90,
   'Kath Treinos — Atleta: Preparação para competição (Bikini)', 10, 25, 0,
   '{"workouts":true,"diet":true,"supplements":true,"juices":true,"chat_sla_h":12,"chat_priority":true,"reavaliation":"weekly","video_call_per_month":1,"whatsapp_daily":true,"photo_analysis":true,"peak_week":true,"competition_planning":true,"stage_coaching":true,"athlete_group":true,"hydration_alerts":true,"daily_motivation":true,"premium_content":true}'::jsonb, 4)
...
create table if not exists public.workout_videos (
  ...
  required_plan    text not null default 'start'
...
create table if not exists public.moto_events (
  ...
  required_plan         text default 'start'
                        check (required_plan in ('start','evolucao','saude_completa','atleta')),
