...
export default async function DiagnosticoTreinosPage() {
  ...
  try {
    ...
    const { data: subRaw } = await userClient.rpc("plan_tier_level", { tier: "free" });
    if (subRaw === null) {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6 text-white">
      <div>
        <h1 className="font-display text-3xl">DIAGNÓSTICO · TREINOS</h1>
        ...
        <Row
          ...
          value={
            published - visible === 0
              ...
              : `${published - visible} ocultos (exigem plano > ${profile?.plan_tier ?? "free"})`
          }
        />
      </Section>
