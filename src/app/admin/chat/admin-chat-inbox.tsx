...
// Respostas rápidas (canned) — sem emojis (regra de produção). Editáveis antes de enviar.
const QUICK_REPLIES = [
  "Oi! Já estou montando seu plano, em breve te aviso.",
  "Vou ajustar e te retorno em seguida.",
  "Pode me mandar uma foto ou vídeo do exercício?",
  "Bora treinar! Qualquer dúvida, estou por aqui.",
];
...
export function AdminChatInbox({
  ...
  // "Não respondidas": tem mensagem não lida OU a última mensagem foi do aluno.
  const filtered = conversations.filter((c) => {
    if (filter === "unanswered" && !(c.unread > 0 || !c.last_from_team)) return false;
    if (query.trim() && !c.full_name.toLowerCase().includes(query.trim().toLowerCase()))
      return false;
    return true;
  });
  const unansweredCount = conversations.filter(
    (c) => c.unread > 0 || !c.last_from_team,
  ).length;
  ...
  return (
    <div className="flex gap-4 h-[calc(100vh-220px)]">
      ...
      >
        {/* Busca + filtro (sticky no topo da lista) */}
        <div className="sticky top-0 z-10 bg-bg-1 border-b border-gray-4/50 p-3 space-y-2">
          <input
            value={query}
          ...
          <div className="flex gap-1.5">
            <button
              ...
              className={cn(
                "px-2.5 py-1 rounded-full text-[10px] font-semibold uppercase tracking-[0.06em] border transition-all",
                filter === "all"
            ...
            >
              Não respondidas{unansweredCount > 0 ? ` (${unansweredCount})` : ""}
            </button>
          </div>
        </div>

        {filtered.length === 0 ? (
          <p className="text-gray-3 text-[13px] text-center py-8">
...
function AdminChatThread({
  ...
  return (
    <>
      ...
      {/* Respostas rápidas — preenchem o campo pra agilizar (editável antes de enviar) */}
      <div className="flex gap-1.5 flex-wrap mt-3">
        {QUICK_REPLIES.map((q) => (
          <button
            ...
            type="button"
            onClick={() => setInput(q)}
            className="text-[11px] text-gray-2 bg-bg-1 border border-gray-4 rounded-full px-2.5 py-1 hover:border-pink/50 hover:text-white transition-colors"
          >
            {q}
          </button>
        ))}
      ...
      <form onSubmit={handleSend} className="mt-2 flex gap-2 items-end">
        <select
          value={senderRole}
