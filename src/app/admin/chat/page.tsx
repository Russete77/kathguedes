...
export default async function AdminChatPage() {
  ...
  const { data: conversations } = await supabase
    .from("messages")
    .select("user_id, body, is_from_kath, is_read, created_at, profiles(full_name, avatar_url)")
    ...
    for (const msg of conversations) {
      if (!grouped.has(msg.user_id)) {
        ...
        grouped.set(msg.user_id, {
          ...
          is_from_kath: msg.is_from_kath,
          unread: 0,
        });
      }
      // Contar não lidas (mensagens do user que a Kath não leu)
      if (!msg.is_from_kath && !msg.is_read) {
