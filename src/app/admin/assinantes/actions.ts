...
export async function setSubscriptionStatus(input: {
  ...
  // Coerência com hasActiveAccess: ativar estende ends_at; cancelar revoga já.
  const statusPatch: {
  ...
  if (data.status === "active") {
    ...
    statusPatch.subscription_ends_at = endsAt.toISOString();
  } else if (data.status === "canceled") {
    statusPatch.subscription_ends_at = new Date().toISOString();
  }
...
export async function setAssinantePlan(input: {
  ...
  // No modelo de 4 tiers, todos são pagos por design.
  // Para cancelar assinatura sem mudar tier, usar setSubscriptionStatus separado.
  // Seta subscription_ends_at (futuro) para coerência com hasActiveAccess.
  const endsAt = new Date();
  endsAt.setFullYear(endsAt.getFullYear() + 1);
