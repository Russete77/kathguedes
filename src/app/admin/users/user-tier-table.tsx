...
const TIER_OPTIONS: { value: PlanTier; label: string; level: number }[] = [
  { value: "start", label: "Start · R$39,90", level: 1 },
  { value: "evolucao", label: "Evolução · R$74,90", level: 2 },
  { value: "saude_completa", label: "Saúde Completa · R$99,90", level: 3 },
  { value: "atleta", label: "Atleta · R$309,90", level: 4 },
];

const TIER_COLOR: Record<string, string> = {
  start: "bg-green-500/15 text-green-300 border-green-500/30",
  evolucao: "bg-yellow-500/15 text-yellow-300 border-yellow-500/30",
  saude_completa: "bg-orange-500/15 text-orange-300 border-orange-500/30",
...
export function UserTierTable({ users }: { users: UserRow[] }) {
  ...
  function handleApply(user: UserRow) {
    const tier = selectedTier[user.id] ?? user.plan_tier ?? "start";
        ...
        <>
          ...
          <div className="sm:hidden space-y-3">
            {filtered.map((u) => (
              <UserCard
                key={u.id}
                user={u}
                selectedTier={selectedTier[u.id] ?? (u.plan_tier as PlanTier) ?? "start"}
          ...
          <div className="hidden sm:block bg-bg-1 border border-gray-4 rounded-[18px] overflow-x-auto">
            <table className="w-full text-sm min-w-[760px]">
              ...
              <tbody>
                {filtered.map((u) => {
                  const current = u.plan_tier ?? "start";
...
function UserCard({
  ...
  const current = user.plan_tier ?? "start";
  return (
