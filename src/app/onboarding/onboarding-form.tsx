...
const interests = [
  { value: "treinos", label: "Biblioteca de Vídeos", desc: "Vídeos de treino HD", icon: BarChart2 },
...
export function OnboardingForm() {
  ...
  // Propaga o destino vindo do registro Clerk (?redirect_url=/planos?autostart=atleta).
  // Sem isso, todo signup cai em /dashboard mesmo vindo da landing com plano escolhido.
  function getRedirectAfter(): string {
    const raw = searchParams?.get("redirect_url");
    if (!raw) return "/dashboard";
    // Aceita só path relativo, evita open redirect.
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      // Navegação "hard": garante que a request já carregue o cookie novo. Não
      // resetamos `loading` no sucesso — a página é trocada em seguida.
      window.location.href = getRedirectAfter();
    } catch (e) {
