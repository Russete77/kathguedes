...
export async function POST(req: NextRequest) {
  try {
    ...
    // 1. Validar cupom (ativo, não expirado, ainda dentro de max_uses, plano OK)
    const { data: coupon, error: couponErr } = await supabase
      .from("coupons")
      .select("id, is_active, max_uses, uses_count, valid_until, required_plan")
      .eq("id", couponId)
      .single();
    ...
    // Enforcement de plano (Crítico 8 da auditoria CTO):
    // Antes o gate era só no frontend/RLS read. Sem check aqui, um user
    // start podia usar cupom required_plan='atleta' chamando direto a API.
    const { data: profile } = await supabase
      .from("profiles")
      .select("plan_tier")
      .eq("id", userId)
      .single();
    const userTier = (profile?.plan_tier as PlanTier | undefined) ?? "start";
    if (!hasPlanAccess(userTier, coupon.required_plan as PlanTier)) {
      return NextResponse.json(
        { error: `Cupom requer plano ${coupon.required_plan}` },
        { status: 403 },
      );
    }
