 ...
 *   Endpoint URL: https://www.kathguedes.com.br/api/webhook/clerk
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Idempotência: Svix reentrega em timeout/erro. Dedup por svix-id usando a
    // mesma tabela do webhook Asaas (payment_id é unique). Sem svix-id (dev sem
    // assinatura), seguimos sem dedup. Reprocessar user.deleted/updated fora de
    // ordem poderia sobrescrever estado mais novo — isto evita.
    const svixId = req.headers.get("svix-id");
    if (svixId) {
      const dedup = createAdminSupabaseClient();
      const { error: dupErr } = await dedup
        .from("webhook_events")
        .insert({ payment_id: `clerk:${svixId}`, event: event.type });
      if (dupErr) {
        if (dupErr.code === "23505") {
          return NextResponse.json({ ok: true, duplicate: true });
        }
        console.error("[webhook/clerk] dedup insert failed:", dupErr.message);
      }
    }
        ...
        .upsert(
          {
            ...
            full_name: fullName,
            plan_tier: "start",
            // canceled = sem acesso pago. Vira "active" quando o webhook
            // PAYMENT_CONFIRMED chegar (handleMensalidadePayment).
            subscription_status: "canceled",
            onboarding_completed: true, // sem onboarding intermediario
          },
