...
const cycleSchema = z.enum(["semestral", "anual", "mensal"]);
...
const subscribeBodySchema = z.object({
  ...
  // Promo opcional. Se vier, server tenta consumir slot atomico via RPC.
  // Se esgotado, segue com preco cheio (sem erro pro user).
  promoSlug: z.string().trim().max(40).optional(),
});
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { plan, cycle, billingType, cpfCnpj: cpfFromBody, promoSlug } = parsed.data;

    const activePlans = await getActivePlans();
    // No modelo de 4 tiers todos têm asaas_value > 0, mas mantém o filtro como defesa.
    const validPaidSlugs = activePlans
      .filter((p) => p.asaas_value > 0)
      .map((p) => p.slug);
    ...
    // currentUser() chama API do Clerk; em prod tem caido com 500 quando ha
    // mismatch de instancia. Tentamos primeiro, e caímos no JWT/Supabase
    // se a API falhar — assim o checkout nao trava por causa do Clerk.
    let clerkEmail: string | null = null;
    try {
      const clerkUser = await currentUser();
      clerkEmail = clerkUser?.emailAddresses[0]?.emailAddress ?? null;
    } catch (e) {
      console.error("[subscribe] currentUser() falhou, usando fallback do JWT:", e);
    }
    // Fallback: pega email do JWT claim se disponivel
    if (!clerkEmail) {
    ...
    return NextResponse.json({
      ...
      chargedValue: result.chargedValue,
      installmentCount: result.installmentCount,
      promoApplied: !!result.promoCodeId,
      message: result.promoCodeId
        ? "Assinatura criada com promocao aplicada! Aguardando confirmacao do pagamento."
        : "Assinatura criada! Aguardando confirmacao do pagamento.",
    });
  } catch (err) {
    // Erros do Asaas viram mensagem acionável (não um 500 opaco).
    if (err instanceof AsaasApiError) {
      // Loga server-side (Vercel) com a causa completa para auditoria.
