...
export default async function PerfilPage() {
  ...
  // retorna 500 (mismatch de instância em prod, instabilidade), evitamos
  // derrubar o /perfil — caímos no profile.full_name do Supabase.
  let user: Awaited<ReturnType<typeof currentUser>> | null = null;
  try {
    user = await currentUser();
  } catch (e) {
    console.error("[perfil] currentUser() falhou, usando fallback do profile:", e);
  }
  ...
  const subscriptionEnds = profile?.subscription_ends_at
    ...
    : null;
  // Tem subscription recorrente ativa (PIX/Boleto) se asaas_subscription_id estiver setado.
  // Cartão de crédito é parcelado (não recorrente) — não gera asaas_subscription_id.
  const hasRecurringSubscription =
    !!(profile as Record<string, unknown>)?.asaas_subscription_id &&
    ...
    <div className="max-w-2xl mx-auto px-4 py-6 space-y-6">
      ...
      <div className="bg-bg-1 border border-gray-4 rounded-[22px] p-6 text-center relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <div className="flex items-center justify-center gap-3 mt-4">
            <Badge variant={isTopPlan(planTier) ? "yellow" : hasPlanAccess(planTier, "evolucao") ? "pink" : "white"}>
              <Crown size={12} />
      ...
      <div className="bg-bg-1 border border-gray-4 rounded-[14px] p-5 space-y-3">
        ...
        <div className="flex items-center justify-between">
          ...
          <Badge variant="pink">{planTier.toUpperCase()}</Badge>
        </div>
        {subscriptionEnds && (
          <div className="flex items-center justify-between">
        ...
        {profile?.subscription_status === "canceled" && subscriptionEnds && (
          <div className="text-[12px] text-yellow/80 bg-yellow/10 rounded-lg px-3 py-2">
            Assinatura cancelada — acesso garantido ate {subscriptionEnds}.
          </div>
        )}
        {hasRecurringSubscription && subscriptionEnds && (
          <div className="pt-1 text-right">
            <CancelSubscriptionButton accessUntil={subscriptionEnds} />
          </div>
        )}
        {!isTopPlan(planTier) && (
      ...
      <div className="bg-bg-1 border border-gray-4 rounded-[14px] overflow-hidden">
        ...
        <ChatMenuItem canChat={hasPlanAccess(planTier, "saude_completa")} />
