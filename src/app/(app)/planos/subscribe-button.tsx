...
import type { Plan, BillingCycle } from "@/lib/billing/plans";
...
export function SubscribeButton({
  ...
  // Autostart: link da landing manda ?autostart=<slug> e abrimos o modal direto
  // para o plano correto após o login/registro do Clerk.
  useEffect(() => {
    if (searchParams?.get("autostart") === plan.slug) {
      setDialogOpen(true);
    }
  ...
  // "Plano Atual" so faz sentido com assinatura ativa. Sem isto, todo usuario
  // recem-cadastrado (plan_tier='start', subscription_status='canceled') via o
  // card START como "Plano Atual" e nao conseguia pagar — perda de pagamentos.
  const isCurrent = currentPlan === plan.slug && hasActiveSubscription;
  // Niveis reais (slugs vigentes). Fonte: PLAN_LEVELS em lib/billing/access.ts.
  const LEVELS: Record<string, number> = {
    free: 0, start: 1, evolucao: 2, saude_completa: 3, atleta: 4,
  };
  // Downgrade so bloqueia quem JA tem assinatura ativa; sem assinatura ativa o
  // usuario pode assinar qualquer plano livremente.
  const isDowngrade =
    hasActiveSubscription && (LEVELS[currentPlan] ?? 0) > plan.level;
            ...
            <>
              ...
              <div className="bg-bg-2/50 border border-gray-4 rounded-lg p-3 space-y-2">
                <p className="text-xs text-gray-3">
                  {cycle === "mensal" ? (
                    <>
                      Plano mensal sem fidelidade. No PIX/boleto a cobranca renova
                      automaticamente todo mes; no cartao e uma cobranca avulsa por mes.
                      O acesso e ativado apos a confirmacao do pagamento.
                    </>
                  ) : (
                    <>
                      Plano {cycle} ({cycle === "anual" ? "12 meses" : "6 meses"}).
                      No PIX/boleto a cobranca e recorrente e renova ao fim do
                      periodo; no cartao voce pode parcelar em ate{" "}
                      {cycle === "anual" ? "12x" : "6x"} (sem renovacao automatica).
                      O acesso e ativado apos a confirmacao do pagamento.
                    </>
                  )}
                </p>
          ...
          {step === "awaiting-payment" && paymentData && (
            <>
              <DialogHeader>
                ...
                {paymentData.chargedValue !== undefined && (
                  <div className={`flex items-center gap-2 mt-2 px-3 py-2 rounded-lg ${paymentData.promoApplied ? "bg-pink/10 border border-pink/30" : "bg-bg-2 border border-gray-4"}`}>
                    <span className="text-xs text-gray-3">Total cobrado:</span>
                    <span className={`text-sm font-bold ${paymentData.promoApplied ? "text-pink" : "text-white"}`}>
                      R$ {paymentData.chargedValue.toFixed(2).replace(".", ",")}
                    </span>
                    {paymentData.promoApplied && (
                      <span className="text-[10px] font-bold text-pink/80 ml-auto">PROMO</span>
                    )}
                  </div>
                )}
              </DialogHeader>
