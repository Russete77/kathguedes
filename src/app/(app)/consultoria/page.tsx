...
import { WorkoutPlanViewer } from "./workout-plan-viewer";
...
export default async function ConsultoriaPage() {
  ...
  const { data: profile } = await supabase
    .from("profiles")
    .select("plan_tier, workout_streak, last_workout_at")
    .eq("id", userId!)
    .single();

  const planTier = profile?.plan_tier || "start";

  // Treinou hoje? (last_workout_at no mesmo dia do calendário)
  const lastWorkoutAt = (profile as { last_workout_at?: string | null } | null)?.last_workout_at;
  const completedToday = lastWorkoutAt
    ? new Date(lastWorkoutAt).toDateString() === new Date().toDateString()
    : false;
  const workoutStreak = (profile as { workout_streak?: number } | null)?.workout_streak ?? 0;
  ...
  if (!consultation) {
    ...
    if (hasPlanAccess(planTier, "evolucao")) {
      return (
  ...
  // Dicas de execução (coach_tips) por youtube_id — buscadas ao vivo da
  // biblioteca, então ficam frescas mesmo em planos já entregues.
  const ytIds = new Set<string>();
  workoutPlan?.weeks?.forEach((w) =>
    w.days?.forEach((d) =>
      d.exercises?.forEach((e) => {
        if (e.youtube_id) ytIds.add(e.youtube_id.replace("short:", ""));
      }),
    ),
  );
  const tipsByYoutubeId: Record<string, string> = {};
  if (ytIds.size > 0) {
    const { data: tipRows } = await supabase
      .from("workout_videos" as never)
      .select("youtube_id, coach_tips" as never)
      .in("youtube_id" as never, Array.from(ytIds) as never);
    const rows = (tipRows ?? []) as unknown as {
      youtube_id: string;
      coach_tips: string | null;
    }[];
    rows.forEach((row) => {
      if (row.coach_tips && row.coach_tips.trim()) {
        tipsByYoutubeId[row.youtube_id] = row.coach_tips;
      }
    });
  }
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6 space-y-10">
      ...
      {/* ══════════════════════════════════════ */}
      {/* ── PLANO DE TREINO (com tabs de semana se periodizado) ── */}
      {/* ══════════════════════════════════════ */}
      {workoutPlan?.weeks && workoutPlan.weeks.length > 0 && (
        <div className="space-y-4">
          <div className="flex justify-end">
            <CompletePlanButton completedToday={completedToday} initialStreak={workoutStreak} />
          </div>
          <WorkoutPlanViewer
            plan={workoutPlan}
            tipsByYoutubeId={tipsByYoutubeId}
            completedDays={
              (Array.isArray((consultation as { completed_days?: unknown }).completed_days)
                ? ((consultation as { completed_days?: unknown }).completed_days as string[])
                : [])
            }
          />
        </div>
      ...
      {dietPlan?.meals && dietPlan.meals.length > 0 && (
        <section className="space-y-5">
          ...
          {/* Total de calorias do dia (soma das refeições) vs meta */}
          {(() => {
            const dayCals = dietPlan.meals.reduce(
              (s, m) => s + (m.foods?.reduce((a, f) => a + (f.calories || 0), 0) ?? 0),
              0,
            );
            if (dayCals <= 0) return null;
            const target = consultation.daily_calories ?? 0;
            const pct = target > 0 ? Math.min(100, Math.round((dayCals / target) * 100)) : 0;
            return (
              <div className="bg-bg-1 border border-gray-4 rounded-[14px] p-4">
                <div className="flex items-center justify-between mb-2">
                  <span className="font-mono text-[11px] text-gray-3 uppercase tracking-wider">
                    Total do dia
                  </span>
                  <span className="font-mono text-[13px] text-emerald-400">
                    {dayCals}{target > 0 ? ` / ${target}` : ""} kcal
                  </span>
                </div>
                {target > 0 && (
