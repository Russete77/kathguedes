  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    ...
    try {
      res = await fetch(`${ASAAS_CONFIG.baseUrl}${endpoint}`, {
        ...
        // Timeout 15s — evita travamento indefinido se Asaas não responder.
        // AbortSignal.timeout dispara TimeoutError (extends DOMException), que
        // cai no catch de networkErr abaixo e ativa o backoff/retry.
...
interface CreatePaymentParams {
  ...
  // Cobrança única usa `value`. Parcelamento usa installmentCount + totalValue
  // (ou installmentValue) e NÃO envia `value` — por isso é opcional.
  value?: number;
  dueDate: string; // YYYY-MM-DD
  description?: string;
  externalReference?: string;
  // ── Parcelamento (cartão) ──
  // Quando informados, o Asaas cria uma cobrança parcelada (installment).
  // totalValue = valor cheio; installmentCount = nº de parcelas.
